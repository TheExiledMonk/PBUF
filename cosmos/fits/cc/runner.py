"""
CC Fit Runner - Execute cosmic chronometer H(z) fits.

This module runs cosmological fits against cosmic chronometer H(z) measurements
for both LCDM and PBUF models. It loads standardized data, computes chi2 values,
...
def load_cc_data():
    """Load standardized CC data from cached npz files."""
    try:
        return load_cc_dataset()
    except FileNotFoundError as exc:
        print(f"⚠️ Could not load CC data: {exc}")
        return None


def run_fit(model_overrides: Optional[Dict[str, Any]] = None, verbose: bool = True):
    """
    Run CC fit for both LCDM and PBUF models.
    ...
    data = load_cc_data()
    if data is None:
        raise FileNotFoundError("Could not load CC data. Run 'python cli.py dataset convert --source cc_cosmic_chronometers_compilation --output data/standardized/cc.npz' first.")

    if verbose:
        print("📊 CC Dataset:")
        print(f"   Name: {data.get('name', 'CC Compilation')}")
    ...
    for model_type in ["lcdm", "pbuf"]:
        ...
        try:
            ...
            # Compute chi2
            chi2 = chi_squared_cc(model, data=data)
    ...
    if "lcdm" in results and "pbuf" in results:
        ...
        if np.isfinite(chi2_lcdm) and np.isfinite(chi2_pbuf):
            ...
            if verbose:
                ...
                if delta_chi2 < 0:
                    print(f"   ✅ PBUF fits CC better than LCDM")
                else:
                    print(f"   ℹ️ LCDM fits CC better (Δchi2 = {delta_chi2:.3f}")
    ...
    for model_type, result in results.items():
        if "error" not in result:
            output_file = results_dir / f"cc_{model_type}.json"
            result["dataset"] = "cc"
