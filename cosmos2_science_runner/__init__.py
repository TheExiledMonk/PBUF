"""Package entry point for the Cosmos2 science-runner CLI."""
...
from cosmos2.science_runner.events import RunEvent
...
def _make_progress_printer(run_name: str):
    def _callback(event: RunEvent) -> None:
        payload = event.payload
        event_type = payload.get("type")
        if event_type == "model_complete":
            model = payload.get("model", "model")
            best = payload.get("best_chi2")
            weighted = payload.get("weighted_chi2")
...
__all__ = ["main", "_make_progress_printer"]
__version__ = "0.0.0"
