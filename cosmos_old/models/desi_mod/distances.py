"""Distance helpers for the DESI modified ΛCDM expansion."""
...
from cosmos.models.desi_mod.expansion import H
...
def H_z(z: float, params: DESIModParams) -> float:
    return H(_scale_factor(z), params)


def comoving_distance(z: float, params: DESIModParams, integrator: Integrator) -> float:
...
def luminosity_distance(z: float, params: DESIModParams, integrator: Integrator) -> float:
...
def angular_diameter_distance(z: float, params: DESIModParams, integrator: Integrator) -> float:
    chi = comoving_distance(z, params, integrator)
    D_M = transverse_comoving_distance(chi, params.H0, params.Omega_k0)
