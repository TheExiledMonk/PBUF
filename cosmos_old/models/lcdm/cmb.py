...
from cosmos.models.lcdm.distances import H as H_of_a
from cosmos.models.lcdm.distances import angular_diameter_distance, comoving_distance
...
def R_b(z: float, params: LCDMParams, T_cmb: float = 2.7255) -> float:
    Og_h2 = Omega_gamma_h2(T_cmb)
    Og0 = Og_h2 / h(params) ** 2
    return 0.75 * (params.Omega_b0 / Og0) / (1.0 + z)


def c_s(z: float, params: LCDMParams, T_cmb: float = 2.7255) -> float:
...
def z_drag_eh(params: LCDMParams) -> float:
    """Approximate drag epoch redshift using Eisenstein & Hu (1998)."""
