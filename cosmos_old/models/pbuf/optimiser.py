"""Per-model optimiser for PBUF parameters."""
...
class PBUFOptimiser:
    """
    Simple basin-walking search restricted to the allowed V11 parameters.

    R_max is now a free macro parameter, so the optimiser varies it together
    with the standard background densities.
    """

    TUNABLE = ("H0", "Omega_m0", "Omega_b0", "Rmax")
    ...
    def search(self, bounds: Bounds, iterations: int = 200) -> OptimiserResult:
        ...
        if best_params is None:
            raise RuntimeError("No candidates evaluated. Check optimiser setup.")

        return OptimiserResult(params=best_params, chi2=best_chi2, evaluations=evals)
