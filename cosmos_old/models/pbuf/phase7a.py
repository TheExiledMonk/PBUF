"""Phase-7a sanity guards for the PBUF cosmology."""
...
from cosmos.models.pbuf.distances import H as pbuf_H_of_a, omega_total_at_a
...
def _build_global_grid(
    ...
    H_values = np.array([pbuf_H_of_a(a, model.params, table) for a in a_grid], dtype=float)
    omega_sigma = np.array([omega_sigma_of_a(a, model.params, table) for a in a_grid], dtype=float)
    omega_total = np.array(
        [omega_total_at_a(a, model.params, table, alpha=model.alpha) for a in a_grid],
        dtype=float,
    )
...
def _check_early_time_lcdm(
    ...
    model: PBUFModel,
    table: ThermalTable,
    lcdm_model_factory: Callable[..., LCDMModel],
) -> None:
    a_grid = np.logspace(-9, -6, 40)
    H_pbuf = np.array([pbuf_H_of_a(a, model.params, table) for a in a_grid], dtype=float)

    try:
        H_lcdm = np.array([lcdm_H_of_a(a, None) for a in a_grid], dtype=float)
    except Exception:
