"""Running-Λ model implementation."""
...
from cosmos.models.running_lambda import cmb as cmb_module
from cosmos.models.running_lambda import growth as rl_growth
from cosmos.models.running_lambda import sanity as rl_sanity
from cosmos.models.running_lambda.distances import H_z as H_z_func
from cosmos.models.running_lambda.distances import comoving_distance
...
class RunningLambdaModel(CosmologyModel):
    """Running-Λ cosmology with GR growth and CMB helpers."""

    def __init__(self, **params: float) -> None:
        coerced = self._coerce_params(params)
        running_params = RunningLambdaParams(**coerced)

        if running_params.Omega_lambda0 is None:
            Omega_lambda = 1.0 - running_params.Omega_m0 - running_params.Omega_r0 - running_params.Omega_k0
            running_params = running_params.with_lambda(Omega_lambda)

        self._params = running_params
        ...
        self._sigma8_today = float(running_params.sigma8_0)
    ...
    @property
    def params(self) -> RunningLambdaParams:
        return self._params
    ...
    def chi2_cmb(self, dataset: Any) -> tuple[CMBOutput, np.ndarray, float]:
        """Compute the CMB χ² for this running-Λ instance with diagnostic logging."""
        ...
        print("RunningLambdaModel.chi2_cmb called with params:", asdict(self._params))
    ...
    def is_valid(self) -> bool:
        """Run the running-Λ sanity suite and return whether it passed."""

        sanitized = {key: float(value) for key, value in self.parameters.items()}
        result = rl_sanity.sanity_checks(sanitized)
        return result.ok
    ...
    def distance_modulus(self, z: float | np.ndarray | list | tuple) -> float | np.ndarray:
        ...
        if not luminosity_distance_non_decreasing(flat, dL_arr):
            self._distance_sanity_ok = False
            if scalar_input:
                return float(np.inf)
    ...
    def _ensure_growth_table(self) -> GrowthTable:
        if self._growth_table is None:
            def rhs(a: float, y: np.ndarray) -> np.ndarray:
                return rl_growth.growth_ode_rhs(a, y, self._params)
    ...
    def _coerce_params(self, params: Dict[str, float]) -> Dict[str, float]:
        required = ["H0", "Omega_m0", "Omega_r0", "Omega_k0", "Omega_b0", "nu_lambda"]
        missing = [key for key in required if key not in params]
        if missing:
            raise ValueError(f"Missing required running-Λ parameters: {missing}")
    ...
    def __repr__(self) -> str:  # pragma: no cover - debugging helper
        return f"RunningLambdaModel({asdict(self._params)})"
