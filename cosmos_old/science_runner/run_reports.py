...
from .utils import ensure_dir, load_json_or_yaml, safe_write_json, serialize_value
...
class ScienceRunReportGenerator:
    ...
    def _collect_summary(self) -> dict[str, Any]:
        ...
        summary["num_data_points"] = self._count_total_data_points(summary.get("datasets", {}).get("fits") or [])
    ...
    def _create_model_plots(
        ...
        if plot_data:
            ...
            if z and h and dm and fs8:
                ...
                self._plt.close(fig)
                plots.append({"label": "prediction curves", "path": self._relative_path(filename)})
    ...
    def _render_html(self, summary: dict[str, Any]) -> str:
        ...
        engine_meta_rows = [
            ("Engine name", meta.get("engine")),
            ("Quantum version", meta.get("quantum_version")),
            ("LUT version", meta.get("lut_version")),
            ("Cosmos commit", meta.get("git_commit")),
        ]
        engine_setting_rows = [
        ...
        for model in models:
            ...
            if plots:
                ...
                lines.append(
                    "<figure>"
                    f"<img src='{_html_escape(path)}' alt='{_html_escape(label)}' loading='lazy'>"
